// File: Commands/ParamOps/GetInstanceParametersBulkCommand.cs
using System;
...
{
    public class GetInstanceParametersBulkCommand : IRevitCommandHandler
    {
        public string CommandName => "get_instance_parameters_bulk";
        ...
        public object Execute(UIApplication uiapp, RequestCommand cmd)
        {
            ...
            else
            {
                ...
                {
                    // Avoid sweeping entire model without a filter
                    return new { ok = false, msg = "Specify elementIds or categories for instance queries." };
                }
            }

            var start = Math.Max(0, p.SelectToken("page.startIndex")?.Value<int?>() ?? 0);
            var batch = Math.Max(1, p.SelectToken("page.batchSize")?.Value<int?>() ?? Math.Min(500, elemIds.Count));
            var total = elemIds.Count;
            var slice = elemIds.Skip(start).Take(batch).ToList();
            ...
            foreach (var id in slice)
            {
                try
                {
                    ...
                    {
                        var (ok, name, val, shown, err) = TryGetParamNormalized(e, k);
                        if (ok)
                        {
                            map[name] = val ?? "";
        ...
        private static (bool ok, string name, object value, string display, string err) TryGetParam(Element e, KeySpec k)
        {
            ...
            try
            {
                ...
                // Heuristic: structural section keys without Spec → treat as length (mm)
                if (p.StorageType == StorageType.Double && (string.IsNullOrEmpty(shown) || v is double) && !string.IsNullOrEmpty(name))
                {
                    var keyL = (name ?? "").Trim().ToLowerInvariant();
