...
{
    /// <summary>
    /// 積算向け：指定フェイス内の複数リージョンについて、
    /// Material / 面積（m2/ft2）/ centroidMm / bboxMm を返却。
    /// 必要に応じて境界ポリライン（軽量化）を返し、Room/Space は複数点（既定5点）で判定。
    /// 材別合計も同梱。
    /// </summary>
    public class GetFaceRegionTakeoffCommand : IRevitCommandHandler
    {
        public string CommandName => "get_face_region_takeoff";
        ...
        public object Execute(UIApplication uiapp, RequestCommand cmd)
        {
            ...
            try
            {
                ...
                bool includeLoops = p.Value<bool?>("includeLoops") ?? false;
                double chordMm = p.Value<double?>("tessellateChordMm") ?? 100.0;
                double simplifyTolMm = p.Value<double?>("simplifyToleranceMm") ?? 20.0;
                int maxPointsPerLoop = Math.Max(50, p.Value<int?>("maxPointsPerLoop") ?? 300);

                int regionOffset = Math.Max(0, p.Value<int?>("regionOffset") ?? 0);
                int regionLimit = Math.Max(1, p.Value<int?>("regionLimit") ?? 50);
                ...
                if (faces.Count == 0)
                {
                    return ResultUtil.Ok(new
                    {
                        ...
                        summary = new { byMaterial = Array.Empty<object>(), totalM2 = 0.0 },
                ...
                var materialSum = new Dictionary<int, (string name, double m2, int count)>();
                double grandM2 = 0.0;

                for (int i = 0; i < slice.Count; i++)
                {
                    var rf = slice[i];
                    var one = BuildRegionTakeoff(doc, elem, rf, regionOffset + i,
                        includeLoops, chordMm, simplifyTolMm, maxPointsPerLoop,
                        includeRoom, includeSpace, rooms, spaces, probeCount, probeStrategy, probeOffsetMm, returnProbeHits);

                    regionsOut.Add(one.obj);

                    int mid = one.materialId;
                    string mname = one.materialName ?? "";
                    double m2 = one.areaM2;
                    grandM2 += m2;
                    if (!materialSum.TryGetValue(mid, out var cur))
                        materialSum[mid] = (mname, m2, 1);
                    else
                        materialSum[mid] = (mname, cur.m2 + m2, cur.count + 1);
                }

                var byMat = materialSum
                    .Select(kv => new { materialId = kv.Key, materialName = kv.Value.name, m2 = kv.Value.m2, count = kv.Value.count })
                ...
                var payload = new
                {
                    ...
                    regions = regionsOut,
                    summary = new { byMaterial = byMat, totalM2 = grandM2 },
                    inputUnits = UnitHelper.DefaultUnitsMeta(),
                    internalUnits = new { Length = "ft", Area = "ft2", Volume = "ft3", Angle = "rad" }
                };
            ...
            {
                RevitLogger.Error($"get_face_region_takeoff failed: {ex}");
                return ResultUtil.Err($"takeoff 失敗: {ex.Message}");
            }
        }

        private static (object obj, int materialId, string materialName, double areaM2) BuildRegionTakeoff(
            Document doc, Element hostElem, Autodesk.Revit.DB.Face rf, int regionIndex,
            bool includeLoops, double chordMm, double simplifyTolMm, int maxPointsPerLoop,
            ...
            int probeCount, string probeStrategy, double probeOffsetMm, bool returnProbeHits)
        {
            ...
            if (mat != null)
            {
                matName = mat.Name ?? ""; matClass = mat.MaterialClass ?? "";
            ...
            if (includeLoops)
            {
                ...
                catch (Exception ex) { RevitLogger.Info($"loops build note region={regionIndex}: {ex.Message}"); }
            }
