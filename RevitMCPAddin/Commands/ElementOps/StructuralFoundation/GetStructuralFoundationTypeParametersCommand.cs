...
namespace RevitMCPAddin.Commands.ElementOps.Foundation
{
    public class GetStructuralFoundationTypeParametersCommand : IRevitCommandHandler
    {
        ...
        public object Execute(UIApplication uiapp, RequestCommand cmd)
        {
            ...
            if (count == 0)
                return new { ok = true, scope = "type", elementId = sourceElementId, typeId = typeElem.Id.IntValue(), uniqueId = typeElem.UniqueId, totalCount, inputUnits = FoundationUnits.InputUnits(), internalUnits = FoundationUnits.InternalUnits() };

            if (namesOnly)
            {
                var names = ordered.Skip(skip).Take(count).Select(pa => pa?.Definition?.Name ?? "").ToList();
                return new { ok = true, scope = "type", elementId = sourceElementId, typeId = typeElem.Id.IntValue(), uniqueId = typeElem.UniqueId, totalCount, names, inputUnits = FoundationUnits.InputUnits(), internalUnits = FoundationUnits.InternalUnits() };
            }
            ...
            return new { ok = true, scope = "type", elementId = sourceElementId, typeId = typeElem.Id.IntValue(), uniqueId = typeElem.UniqueId, totalCount, parameters = list, inputUnits = FoundationUnits.InputUnits(), internalUnits = FoundationUnits.InternalUnits() };
        }
    }
}
