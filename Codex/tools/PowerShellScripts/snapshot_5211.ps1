# @feature: snapshot 5211 | keywords: 柱, ビュー, スナップショット
param(
  [int]$WaitSec = 240,
...
$run = Join-Path $work ("SnapshotRun_5211_"+$stamp)
...
if($vid -le 0){ throw 'Could not resolve RSL1 viewId on 5211' }

# Snapshot (structural framing/columns with analytic wire)
$out = Join-Path $run 'snap_5211_RSL1.json'
