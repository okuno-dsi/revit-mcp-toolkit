# @feature: create seed and type views typeid | keywords: 壁, スペース, ビュー
param(
  [int]$Port = 5210,
  [int]$BatchSize = 800,
...
$MANIFEST = Join-Path $LOGS 'create_seed_and_type_views_typeid.manifest.json'
...
function Rename-View([int]$viewId,[string]$newName){
  foreach($pn in @('View Name','ビュー名')){
    try{
      ...
      if($jr -and $jr.ok){ return $true }
    } catch {}
  }
  return $false
}

function Get-WallTypeGroupsByTypeId([int]$viewId){
  $WALL = -2000011
  ...
  for($i=0; $i -lt $wallIds.Count; $i+=$chunk){
    ...
    foreach($e in $elems){
      ...
      if($eid -le 0 -or -not $tid){ continue }
      ...
      [void]$groups[[string]$tid].elementIds.Add($eid)
    }
  }
  return $groups
}

function Isolate-By-TypeId([int]$viewId,[int]$typeId,[int[]]$allWallIds,[hashtable]$groupsMap){
  $filter = @{ includeClasses=@('Wall'); includeCategoryIds=@(-2000011); parameterRules=@(@{ target='instance'; builtInName='ELEM_TYPE_PARAM'; op='eq'; value=$typeId }); logic='all' }
  ...
  $targetIds = @($groupsMap[[string]$typeId].elementIds)
...
# Type-specific views by TypeId
Write-Host '[Types] Grouping walls by typeId in active view' -ForegroundColor Cyan
$groups = Get-WallTypeGroupsByTypeId -viewId $activeId
$keys = @($groups.Keys)
...
$typeViews = @()
foreach($tidKey in $keys){
  $g = $groups[[string]$tidKey]
  $tn = [string]$g.typeName
  $tid = [int]$g.typeId
  $vid = $null
  if($ReuseExisting -and (Test-Path $MANIFEST)){
    try{
      ...
      foreach($tv in @($mf2.typeViews)){
        if(([int]$tv.typeId) -eq $tid){ $vid = [int]$tv.viewId; break }
      }
    }catch{}
  }
  if(-not $vid){
    Write-Host ("[Type] Duplicating: {0} (typeId={1})" -f $tn, $tid) -ForegroundColor Cyan
    $vid = Duplicate-View -viewId $activeId -prefix ''
  } else {
    Write-Host ("[Type] Reusing existing viewId={0} for '{1}' (typeId={2})" -f $vid, $tn, $tid) -ForegroundColor Cyan
  }
  Reset-View -viewId $vid
  $allWallIds = @(); foreach($arr in $groups.Values){ $allWallIds += @($arr.elementIds) }
  $null = Isolate-By-TypeId -viewId $vid -typeId $tid -allWallIds $allWallIds -groupsMap $groups
  $ren = Rename-View -viewId $vid -newName $tn
  Write-Host ("  -> viewId={0} name='{1}' (rename {2})" -f $vid, $tn, ($ren ? 'ok' : 'skipped')) -ForegroundColor Green
  $typeViews += @{ name=$tn; viewId=$vid; typeId=$tid }
}
