# @feature: export floor types to excel | keywords: 柱, 壁, Excel, マテリアル, 床
param(
  [int]$Port = 5210,
  [string]$OutPath,
...
function Try-WriteExcel {
  ...
  try {
    ...
    $ws1 = $wb.Worksheets.Item(1); $ws1.Name = 'TypeParameters'
    $ws2 = $wb.Worksheets.Item(2); $ws2.Name = 'LayerStructure'
...
# Minimal XLSX writer fallback (same as walls)
...
function Write-XlsxZip {
  param([object[]]$ParamsRows,[object[]]$LayerRows,[string]$OutPath)
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>Floor Types Export</dc:title>
...
if (-not $OutPath) { $OutPath = Join-Path $baseDir ("FloorTypes_{0}.xlsx" -f $ts) }
...
if ($ReuseCsv) {
  ...
  $latestParams = Get-ChildItem -Path $baseDir -Filter 'FloorTypes_*_TypeParameters.csv' | Sort-Object LastWriteTime -Descending | Select-Object -First 1
  $latestLayers = Get-ChildItem -Path $baseDir -Filter 'FloorTypes_*_LayerStructure.csv' | Sort-Object LastWriteTime -Descending | Select-Object -First 1
  if (-not $latestParams -or -not $latestLayers) { throw "No existing FloorTypes CSVs found in $baseDir. Omit -ReuseCsv to fetch from Revit." }
  ...
  Write-Host "[1/4] Fetching floor types..." -ForegroundColor Cyan
  $typesRes = Invoke-RevitCommandJson -Method 'get_floor_types' -Params @{} -Port $Port
  if (-not $typesRes.ok) { throw "get_floor_types failed" }
  $types = @()
  if ($typesRes.types) { $types = @($typesRes.types) }
  elseif ($typesRes.floorTypes) { $types = @($typesRes.floorTypes) }
  if (-not $types -or $types.Count -eq 0) { throw "No floor types found." }

  Write-Host ("[2/4] Fetching parameters and layers for {0} floor types..." -f $types.Count) -ForegroundColor Cyan
  foreach ($t in $types) {
    $tid = [int]$t.typeId
    $tname = [string]$t.typeName
    $pr = Invoke-RevitCommandJson -Method 'get_floor_type_parameters' -Params @{ typeId = $tid } -Port $Port
    ...
    $lr = Invoke-RevitCommandJson -Method 'get_floor_layers' -Params @{ typeId = $tid } -Port $Port
