# @feature: update wall parameter safe | keywords: 壁, スペース, ビュー
param(
  [int]$Port = 5210,
  [int]$ElementId,
  [string]$Param = 'Comments',
  [string]$Value = 'SmokeTest',
  [switch]$Force,
  [switch]$DryRun
)
...
if([string]::IsNullOrWhiteSpace($Param)){ Write-Error "Param must be specified (e.g., 'Comments')."; exit 2 }

# Build smoke_test payload (update_wall_parameter expects paramName / builtInName / builtInId / guid)
$smokeObj = @{ method = 'update_wall_parameter'; params = @{ elementId = $ElementId; paramName = $Param; value = $Value } }
...
Write-Host "[smoke_test] update_wall_parameter elementId=$ElementId param=$Param" -ForegroundColor Cyan
$drySmokePath = Join-Path $LOGS 'update_wall_parameter.smoke.dryrun.json'
...
$smokePath = Join-Path $LOGS 'update_wall_parameter.smoke.json'
...
$execObj = @{ elementId = $ElementId; paramName = $Param; value = $Value; __smoke_ok=$true }
$execJson = ($execObj | ConvertTo-Json -Compress -Depth 6)
Write-Host "[execute] update_wall_parameter elementId=$ElementId param=$Param" -ForegroundColor Yellow
$execOut = & python $PY --port $Port --command update_wall_parameter --params $execJson --wait-seconds 120
$execPath = Join-Path $LOGS 'update_wall_parameter.exec.json'
