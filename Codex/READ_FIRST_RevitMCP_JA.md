...
```
pwsh -File Scripts/Reference/test_connection.ps1 -Port 5210
```
- 出力: `Projects/<ProjectName>_<Port>/Logs/agent_bootstrap.json`
...
取得例（名前だけの簡易一覧）:
```
python Scripts/Reference/send_revit_command_durable.py --port 5210 --command list_commands --params '{"namesOnly":true}' --output-file Projects/<ProjectName>_<Port>/Logs/list_commands_names.json
```
...
```
pwsh -File Scripts/Reference/list_elements_in_view.ps1 -Port 5210
```
- 出力: `Projects/<ProjectName>_<Port>/Logs/elements_in_view.json`（IDは `result.result.rows`）
- Python で直接取得（IDs only, 上限200件）
```
python Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_elements_in_view --params "{\"viewId\": <activeViewId>, \"_shape\":{\"idsOnly\":true,\"page\":{\"limit\":200}}}" --output-file Projects/<ProjectName>_<Port>/Logs/elements_in_view.json
```
...
```
# まずは DryRun で送信内容を確認
pwsh -File Scripts/Reference/set_visual_override_safe.ps1 -Port 5210 -ElementId <id> -DryRun

# 問題なければ実行（必要に応じて -Force）
pwsh -File Scripts/Reference/set_visual_override_safe.ps1 -Port 5210 -ElementId <id>
```
2) 壁パラメータ更新の安全実行（コメント推奨）
```
pwsh -File Scripts/Reference/update_wall_parameter_safe.ps1 -Port 5210 -ElementId <id> -Param Comments -Value "Test via smoke" -DryRun
pwsh -File Scripts/Reference/update_wall_parameter_safe.ps1 -Port 5210 -ElementId <id> -Param Comments -Value "Test via smoke"
```
...
- 例（単要素に色/透明度）
```
python Scripts/Reference/send_revit_command_durable.py --port 5210 --command set_visual_override --params "{\"elementId\":<id>,\"r\":0,\"g\":200,\"b\":255,\"transparency\":20,\"autoWorkingView\":true,\"__smoke_ok\":true}"
```
...
## 6) 作業ルール（最短で迷わない）
- プロジェクト別に `Projects/<ProjectName>_<ProjectID>` を作成し、その下で成果物・ログを管理。
- 参照系は `Docs/Manuals/`、編集や実験ログは `Projects/` 配下へ。
- 書き込みは常に `*_safe.ps1` を最優先（`-DryRun` で事前確認）。
- 単位: 長さ=mm, 角度=deg（サーバー側で内部変換）。

リビジョン操作のまとめ: `Docs/Manuals/RevisionOps.md` を参照（一覧/シート関連/更新/クラウド作成）。
- レスポンス: JSON-RPC エンベロープ内の `result.result.*` を読む。
- 共通ステータス: `result.result.ok`, `result.result.code`, `result.result.msg`, `result.result.timings.totalMs`（詳細: `Docs/Manuals/Response_Envelope_JA.md`）。
...
## 9) 次に読むと良い原典
- 最短クイックスタート: `Docs/Manuals/ConnectionGuide/QUICKSTART.md`
- コマンド要点: `Docs/Manuals/Commands/HOT_COMMANDS.md`
- スクリプト一覧: `Scripts/Reference/README.md`
- 可視化の更新版ガイド: `Docs/Manuals/UPDATED_VIEW_VISUALIZATION_COMMANDS_EN.md`

## 10) 実行ポリシー（Windows/PowerShell）重要
- 署名されていない `*.ps1` はブロックされる場合があります。実行時だけ Bypass を付けてください。
  - 例: `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/test_connection.ps1 -Port 5210`
  - 例: `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/export_walls_by_type_simple.ps1 -Port 5210`
- 詳細: `Docs/Manuals/ExecutionPolicy_Windows.md`

## 11) 高速DWG出力（seed + 壁タイプ別）
- ビューを直接いじらず、`export_dwg { elementIds: [...] }` で複製ビューに隔離して出力（高信頼）。
- 実行例（最新プロジェクト自動検出）:
  - `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/export_walls_by_type_simple.ps1 -Port 5210`
