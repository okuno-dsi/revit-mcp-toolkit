...
```
- Output: `Projects/<ProjectName>_<Port>/Logs/agent_bootstrap.json`
...
```
- Output: `Projects/<ProjectName>_<Port>/Logs/elements_in_view.json` (IDs at `result.result.rows`)
...
- Create `Projects/<ProjectName>_<ProjectID>` per project and store all artifacts/logs there.
- Reference docs live under `Docs/Manuals/`; edits and experiment logs go under `Projects/`.
...
- Standard status fields: `result.result.ok`, `result.result.code`, `result.result.msg`, `result.result.timings.totalMs` (see `Docs/Manuals/Response_Envelope_EN.md`).
...
- Quickstart shortest path: `Docs/Manuals/ConnectionGuide/QUICKSTART.md`
- Hot commands digest: `Docs/Manuals/Commands/HOT_COMMANDS.md`
- Script list: `Scripts/Reference/README.md`
- Updated visuals guide: `Docs/Manuals/UPDATED_VIEW_VISUALIZATION_COMMANDS_EN.md`

## 10) Execution Policy (Windows/PowerShell) — Important
- Unsigned `*.ps1` may be blocked. Use **Bypass** only for the run.
  - Example: `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/test_connection.ps1 -Port 5210`
  - Example: `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/export_walls_by_type_simple.ps1 -Port 5210`
- See: `Docs/Manuals/ExecutionPolicy_Windows.md`
