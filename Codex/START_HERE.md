...
## 4.7) パフォーマンス注意（重要）
- 同一対象への更新を **1件ずつ多数回** 送るのは避けてください（例: `set_family_type_parameter` の連続実行）。
- 原則として、可能な場合は **一括更新コマンド** を優先してください（例: `update_parameters_batch`）。
- 目安:
  - 更新対象が複数（例: 同一要素/タイプに 5 項目以上、または要素数が 10 以上）の場合は、まずバッチ化を検討。
  - ループ内で `rpc` を多発させる前に、まとめて送れる形（items配列）へ変換。
- 効果:
  - 往復回数とトランザクション回数を削減し、タイムアウト/停止リスクを低減できます。
  - Python Runner でも体感速度が大きく改善します。
