# @feature: 柱リスト用ファミリを電算CSVから自動判定で同期（Dynamo代替） | keywords: 柱リスト, CSV, RC, SRC, 同期
# -*- coding: utf-8 -*-

"""
柱リスト用ファミリ同期（自動判定）スクリプト。
- family_mode=auto で RC/SRC を自動判定
- 必要なら family_mode を rc/src/both に変更して使えます
"""
...
def main() -> int:
    ...
    fm = str(FAMILY_MODE or "auto").lower().strip()
