...
本書は Revit MCP Add-in の HTML 出力系コマンドのうち、ダッシュボード出力の使い方をまとめたものです。
...
例（HTTP シンプルルート）
- PowerShell
  - `Invoke-RestMethod 'http://127.0.0.1:5210/rpc/export_dashboard_html' -Method Post -ContentType 'application/json; charset=utf-8' -Body (@{ outDir = '%USERPROFILE%\Documents\Revit_MCP\Source\Codex\Projects\\DashboardOut' } | ConvertTo-Json -Compress)`

例（JSON-RPC）
- PowerShell
  - `Invoke-RestMethod 'http://127.0.0.1:5210/rpc' -Method Post -ContentType 'application/json; charset=utf-8' -Body (@{ jsonrpc='2.0'; id='1'; method='export_dashboard_html'; params=@{ outDir='%USERPROFILE%\Documents\Revit_MCP\Source\Codex\Projects\\DashboardOut' } } | ConvertTo-Json -Compress)`

JSONL 一行例
- `{ "jsonrpc":"2.0","id":"1","method":"export_dashboard_html","params":{"outDir":"%USERPROFILE%\\Documents\\Revit_MCP\\Projects\\DashboardOut"} }`
