...
*   `Python/Scripts/Reference/send_revit_command_durable.py` スクリプトが利用可能であること。
...
```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_wall_types
```

### 3.2. レベル情報の一覧取得

```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_levels
```
...
```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command create_wall --params "{\"start\":{\"x\":0.0,\"y\":0.0,\"z\":0.0},\"end\":{\"x\":5000.0,\"y\":0.0,\"z\":0.0},\"baseLevelId\":279461,\"wallTypeId\":103484,\"heightMm\":3000}"
```
...
```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command create_wall --params "{\"start\":{\"x\":0.0,\"y\":10000.0,\"z\":0.0},\"end\":{\"x\":5000.0,\"y\":10000.0,\"z\":0.0},\"baseLevelId\":279461,\"wallTypeId\":103484,\"heightMm\":\"level-to-level\"}"
```
...
```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command create_wall --params "{\"start\":{\"x\":0.0,\"y\":20000.0,\"z\":0.0},\"end\":{\"x\":5000.0,\"y\":20000.0,\"z\":0.0},\"baseLevelId\":279461,\"wallTypeId\":103484,\"topLevelId\":279483}"
```
...
```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command create_wall --params "{\"start\":{\"x\":0.0,\"y\":30000.0,\"z\":0.0},\"end\":{\"x\":5000.0,\"y\":30000.0,\"z\":0.0},\"baseLevelId\":279461,\"wallTypeId\":103484,\"topLevelId\":279472,\"baseOffsetMm\":100,\"topOffsetMm\":-50}"
```
...
sys.path.append(os.path.join(os.path.dirname(__file__), 'Python')) # Scripts/Reference/send_revit_command_durable.py があるディレクトリをパスに追加
...
## 5. 既存壁に密着する壁の作成（`element.create_flush_walls`）

`element.create_flush_walls` は、既存壁の指定側に「面合わせ」で別タイプの壁を作成します（仕上げ壁の重ね壁用途など）。

ポイント
- `sourceWallIds` を省略/空にすると、**現在選択中の Wall のみ**を対象にします。
- 「-Y 側の面に密着」させたい場合は、`sideMode=ByGlobalDirection` と `globalDirection=[0,-1,0]` を使います（グローバル座標基準）。
- 上下拘束（レベル/オフセット/高さ）は `copyVerticalConstraints=true` で複製します。
  - 注意: **Attach Top/Base（アタッチ）** は複製しません（Revit 2023 の制約）。

### 5.1. 使用例: 選択中の壁の -Y 側に、指定タイプの壁を密着作成

```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command element.create_flush_walls --params "{\"newWallTypeNameOrId\":\"(内壁)W5\",\"sideMode\":\"ByGlobalDirection\",\"globalDirection\":[0,-1,0],\"sourcePlane\":\"FinishFace\",\"newPlane\":\"FinishFace\",\"newExteriorMode\":\"MatchSourceExterior\",\"miterJoints\":true,\"copyVerticalConstraints\":true}"
```

### 5.2. 部屋の辺の長さ範囲だけで仕上げ壁を作る（`room.apply_finish_wall_type_on_room_boundary`）

`element.create_flush_walls` は、基本的に **ソース壁の LocationCurve 全長**を基準に「面合わせ壁」を作成します。  
壁が複数の部屋にまたがっていて「選択した部屋の辺の範囲だけ」で仕上げ壁を作りたい場合は、部屋境界セグメント長で作成する `room.apply_finish_wall_type_on_room_boundary` を使います。

```bash
python Python/Scripts/Reference/send_revit_command_durable.py --port 5210 --command room.apply_finish_wall_type_on_room_boundary --params "{\"fromSelection\":true,\"newWallTypeNameOrId\":\"(内壁)W5\",\"onlyRcCore\":true,\"coreMaterialNameContains\":\"*コンクリート\",\"boundaryLocation\":\"Finish\",\"skipExisting\":true,\"joinEnds\":true}"
```
...
```bash
del %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_create_4_walls_v2.py \
    %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\created_4_walls_v2.json \
    %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_create_walls_level_to_level.py \
    %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\created_walls_level_to_level.json \
    %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_offset_walls_level_to_level.py \
    %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_create_dimension_between_walls_level_to_level.py
```
