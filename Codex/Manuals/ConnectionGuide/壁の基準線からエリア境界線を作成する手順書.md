...
- `Python/Scripts/Reference/send_revit_command_durable.py` スクリプトが正しく配置されていること。
...
```bash
python Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_rooms --output-file all_rooms_for_boundary.json
```
...
def run_revit_command(port, command, params=None, temp_id=0):
    script_path = os.path.join('Python', 'Scripts/Reference/send_revit_command_durable.py')
