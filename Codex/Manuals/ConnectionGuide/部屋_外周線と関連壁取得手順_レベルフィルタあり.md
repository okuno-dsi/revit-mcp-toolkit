...
- JSON-RPC クライアントとして `Manuals/Scripts/send_revit_command_durable.py` を使用。
...
python -X utf8 Manuals/Scripts/send_revit_command_durable.py --port <PORT> --command get_elements_in_view --params $params --output-file temp_mcp/rooms.json
$ids = (Get-Content -Raw temp_mcp/rooms.json | ConvertFrom-Json).result.elementIds
foreach($id in $ids){
  $p = @{ elementId = [int]$id } | ConvertTo-Json -Compress
  $r = python -X utf8 Manuals/Scripts/send_revit_command_durable.py --port <PORT> --command get_element_info --params $p | ConvertFrom-Json
...
python -X utf8 Manuals/Scripts/send_revit_command_durable.py --port <PORT> --command get_room_params --params $p --output-file temp_mcp/room_params.json
...
python -X utf8 Manuals/Scripts/send_revit_command_durable.py --port <PORT> --command get_room_boundary --params $p --output-file temp_mcp/room_boundary.json
```
...
python -X utf8 Manuals/Scripts/send_revit_command_durable.py --port <PORT> --command get_walls --params $p --output-file temp_mcp/walls_level_filtered.json
```
...
for($i=0; $i -lt $boundary.Count; $i++){
  ...
  foreach($w in $walls){
    ...
    $resp = python -X utf8 Manuals/Scripts/send_revit_command_durable.py --port <PORT> --command analyze_segments --params $params | ConvertFrom-Json
