...
- JSON-RPC クライアント: `Scripts/Reference/send_revit_command_durable.py`
...
python -X utf8 Scripts/Reference/send_revit_command_durable.py --port <PORT> --command get_rooms --params $pRooms --output-file temp_mcp/get_rooms_full.json
...
function Get-RoomMeta([int]$roomId){
  $rp = python -X utf8 Scripts/Reference/send_revit_command_durable.py --port <PORT> --command get_room_params --params (@{ elementId=$roomId; skip=0; count=300 } | ConvertTo-Json -Compress) | ConvertFrom-Json
...
python -X utf8 Scripts/Reference/send_revit_command_durable.py --port <PORT> --command get_ceilings --params $pCeil --output-file temp_mcp/get_ceilings_all.json
...
foreach($c in $ceils){
  ...
  $resp = python -X utf8 Scripts/Reference/send_revit_command_durable.py --port <PORT> --command get_ceiling_boundaries --params $p | ConvertFrom-Json
...
```powershell
$rb = python -X utf8 Scripts/Reference/send_revit_command_durable.py --port <PORT> --command get_room_boundary --params (@{ elementId=$rid } | ConvertTo-Json -Compress) | ConvertFrom-Json
