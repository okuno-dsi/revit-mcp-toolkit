...
1.  **Revitのレベル情報を取得**:
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\Python\Manuals/Scripts/send_revit_command_durable.py --port 5210 --command get_levels --params "{}"
    ```
...
2.  **平面図ビューの作成**:
    ...
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\Python\Manuals/Scripts/send_revit_command_durable.py --port 5210 --command create_view_plan --params "{\"name\": \"Gemini_1F_Plan\", \"levelName\": \"1FL\"}"
    ```
...
1.  **グリッド情報の確認**:
    ...
    ```bash
    type %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\grids.json
    ```
...
2.  **寸法タイプIDの選択**:
    ...
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\Python\Manuals/Scripts/send_revit_command_durable.py --port 5210 --command get_dimension_types --params "{}"
    ```
...
3.  **寸法作成スクリプトの実行**:
    ...
    sys.path.append(os.path.join('%USERPROFILE%/Documents/VS2022/Ver101/GeminiCLI', 'Python'))
    ...
    with open("%USERPROFILE%/Documents/VS2022/Ver101/GeminiCLI/created_grid_dimensions.json", "w", encoding="utf-8") as f:
        json.dump(all_dimension_ids, f, indent=2, ensure_ascii=False)
    ```

    実行コマンド:
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\temp_create_dimensions_for_grids.py
    ```
...
1.  **寸法移動スクリプトの実行**:
    ...
    # 作成された寸法IDを読み込む
    with open("%USERPROFILE%/Documents/VS2022/Ver101/GeminiCLI/created_grid_dimensions.json", "r", encoding="utf-8") as f:
    ...
    実行コマンド:
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\temp_move_dimensions.py
    ```
...
```bash
del %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\temp_create_dimensions_for_grids.py %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\created_grid_dimensions.json %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\temp_move_dimensions.py
```
