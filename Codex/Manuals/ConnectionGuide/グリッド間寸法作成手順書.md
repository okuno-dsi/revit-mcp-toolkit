...
1.  **Revitのレベル情報を取得**:
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_levels --params "{}"
    ```
...
2.  **平面図ビューの作成**:
    ...
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command create_view_plan --params "{\"name\": \"Gemini_1F_Plan\", \"levelName\": \"1FL\"}"
    ```
...
2.  **寸法タイプIDの選択**:
    ...
    ```bash
    python %USERPROFILE%\Documents\VS2022\Ver101\GeminiCLI\Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_dimension_types --params "{}"
    ```
