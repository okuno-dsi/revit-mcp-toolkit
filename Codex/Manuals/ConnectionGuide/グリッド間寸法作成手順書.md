...
1.  **Revitのレベル情報を取得**:
    ```bash
    python %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_levels --params "{}"
    ```
...
2.  **平面図ビューの作成**:
    ...
    ```bash
    python %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command create_view_plan --params "{\"name\": \"Gemini_1F_Plan\", \"levelName\": \"1FL\"}"
    ```
...
1.  **グリッド情報の確認**:
    ...
    ```bash
    type %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\grids.json
    ```
...
2.  **寸法タイプIDの選択**:
    ...
    ```bash
    python %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\Python\Scripts/Reference/send_revit_command_durable.py --port 5210 --command get_dimension_types --params "{}"
    ```
...
3.  **寸法作成スクリプトの実行**:
    ...
    実行コマンド:
    ```bash
    python %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_create_dimensions_for_grids.py
    ```
...
1.  **寸法移動スクリプトの実行**:
    ...
    実行コマンド:
    ```bash
    python %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_move_dimensions.py
    ```
...
```bash
del %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_create_dimensions_for_grids.py %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\created_grid_dimensions.json %USERPROFILE%\Documents\Revit_MCP\Source\GeminiCLI\temp_move_dimensions.py
```
