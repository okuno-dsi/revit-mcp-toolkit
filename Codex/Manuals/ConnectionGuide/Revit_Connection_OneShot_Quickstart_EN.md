...
Client → Playbook (C# .NET 8) → RevitMCPServer (inside Revit)
...
## 4) Send JSON‑RPC to Revit via the Chain

Two patterns are supported:

1) Fixed forward (Playbook `--forward` points to 5210):

- Client → `POST http://127.0.0.1:5209/rpc` (Playbook forwards to 5210)

2) Per‑request targeting (recommended for multiple Revit instances):

- Client → `POST http://127.0.0.1:5209/t/5211/rpc` (targets Revit at 5211)
...
## 5) Caching: Avoid Re‑querying Heavy Models
...
````powershell
python "C:\Users\okuno\Documents\VS2022\Ver431\Codex\Manuals\Scripts\cache_revit_info.py" --proxy http://127.0.0.1:5209 --revit-port 5211 --out-dir "C:\Users\okuno\Documents\VS2022\Ver431\Codex\Manuals\Logs" --ttl-sec 0
````

- Use cached data (concise summary):

````powershell
powershell -File "C:\Users\okuno\Documents\VS2022\Ver431\Codex\Manuals\Scripts\get_project_and_documents_cached.ps1" -Port 5211 -Proxy http://127.0.0.1:5209 -OutDir "C:\Users\okuno\Documents\VS2022\Ver431\Codex\Manuals\Logs"
````
...
- WinError 10013 / AddressInUse: port conflict. Keep Playbook on 5209, and Revit on 5210/5211/5212.
...
3) Send RPC to a specific Revit instance (example: 5211):
...
````powershell
Get-NetTCPConnection -LocalPort 5209 | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force }
````
