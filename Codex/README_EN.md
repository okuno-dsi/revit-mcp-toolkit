...
Quickstart
- Test connection: `Manuals/Scripts/test_connection.ps1 -Port 5210`
- Get levels: `Manuals/Scripts/get_levels.ps1 -Port 5210`
  - IDs only: `Manuals/Scripts/get_levels.ps1 -Port 5210 -IdsOnly`
- Results save under `Work/<ProjectName>_<Port>/Logs/` (e.g., `levels.json`).
- Quick check: `Manuals/Scripts/test_connection.ps1 -Port 5210`
- Durable command sender (recommended):
  - `python Manuals/Scripts/send_revit_command_durable.py --port 5210 --command ping_server`
- Project working root: `Work/<ProjectName>_<ProjectID>`
- Manuals folder is for guides only; save runtime outputs under `Work/` per project.
- Use safe scripts for writes: `*_safe.ps1` (supports `-DryRun`)

See `Manuals/` for detailed guides and command references.
Revision features: see `Manuals/RevisionOps.md` for listing, updating, cloud, and circle helpers.
Lightweight reads: see `Manuals/ShapeAndPaging.md` for `_shape`/`summaryOnly`.
...
- Save the current registry for auditing:
  `python Manuals/Scripts/send_revit_command_durable.py --port 5210 --command list_commands --params '{"namesOnly":true}' --output-file Work/<Project>_<Port>/Logs/list_commands_names.json`
