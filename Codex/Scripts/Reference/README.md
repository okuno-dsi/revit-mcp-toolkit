...
- test_connection.ps1: Ping and bootstrap; saves Projects/<Project>_<Port>/Logs/agent_bootstrap.json
- test_terminology_routing.ps1: Validates terminology-aware `help.search_commands` ranking (断面/平断面/立面/RCP)
- list_elements_in_view.ps1: Uses activeViewId to list element IDs; saves Projects/<Project>_<Port>/Logs/elements_in_view.json
...
Logs location
- Scripts now prefer `Projects/<ProjectName>_<Port>/Logs/` for JSON outputs. They will fall back to `Docs/Manuals/Logs/` if needed for compatibility.
...
- `list_elements_in_view.ps1` reads the active view ID from `Projects/<Project>_<Port>/Logs/agent_bootstrap.json` (fallback: `Docs/Manuals/Logs/agent_bootstrap.json`) at `result.result.environment.activeViewId`.
- `set_visual_override.ps1` chooses the first valid (>0) element from `elements_in_view.json` in `Projects/<Project>_<Port>/Logs/` (fallback: `Docs/Manuals/Logs/`), or you can pass `-ElementId`.
...
- See: `Docs/Manuals/AutoCadOut/DWG_Layer_Split_Export_EN.md` for the generic pattern.
- Wall-by-type example to latest Project_* folder:
  `pwsh -File Scripts/Reference/export_walls_by_type_snapshot.ps1 -Port 5210`

Schedule CSV Exports
- export_all_schedules_to_csv.ps1: Bulk-export all schedules to CSV.
  - As‑Is (as-displayed):
    `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/export_all_schedules_to_csv.ps1 -Port 5210 -OutDir "Projects/<Project>_<Port>/Schedules_Compare/AsIs"`
  - Itemize + Fill (value semantics):
    `pwsh -ExecutionPolicy Bypass -File Scripts/Reference/export_all_schedules_to_csv.ps1 -Port 5210 -OutDir "Projects/<Project>_<Port>/Schedules_Compare/Itemize_Fill" -FillBlanks -Itemize`
