...
> **このREADMEは「刺さる順」に並べ替えた叩き台です。**  
> TODO コメント（`TODO:`）を埋めるだけで、公開品質に近づく構成にしています。
...
> TODO: 「実際に提供されているコマンド一覧」へのリンク（または自動生成）を追加  
> TODO: 代表的な“成功するプロンプト例”を3つ載せる（短く）

---

## デモ（ここが刺さる：1枚でわかる）

> TODO: スクリーンショット/GIF を追加（例：AIがモデル情報を取得→差分提示→確認→適用）
...
> TODO: ここに「よくある躓き（権限/パス/実行ポリシー）」を3つだけ載せる  
> TODO: “確認コマンド（ヘルスチェック）” があるなら1行で書く
...
> TODO: 実装が確定している“通信方式”があるなら明記（例：ローカルのみ、ポート固定/可変、等）
...
> TODO: “開発者向け（ビルドに必要なSDK）” と “利用者向け（Runtimeだけ）” を分けて書く
...
> TODO: 可能なら「危険操作のガード（削除/一括変更）」の設計方針を明記
...
> TODO: “最初のPR向け（good first issue）” の運用をするならここに方針を書く
