...
- **Codex GUI 強化**: 入力欄の縦リサイズ、タスクバーでの実行中/完了の視認性向上。
- **Python Runner（初登場）**: 専用Pythonスクリプトでの実行を効率化（保存先統一・出力改善・コマンド名強調を含む）。
- **無人運用の強化**: ダイアログ自動Dismiss＋キャプチャ/OCR（best effort）。
- **空間系の一括取得**: Room/Space/Area の bulk 取得とパラメータ提案の追加。
- **Dynamo 実行**: コマンドは追加済みだが **不安定なため原則推奨しません**。
...
## クレジット
- Created by **奥野哲也** and **GPT series** (2025)
