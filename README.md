...
- **Python Runner（初登場）**：専用Pythonスクリプトを Revit 内で実行でき、作業効率を大幅に向上。
- **Codex GUI 強化**：入力欄の縦リサイズ、実行中/完了のタスクバー通知で操作性が改善。
- **無人運用の安定化**：ダイアログ自動Dismiss＋キャプチャ/OCR（best effort）で停止リスクを軽減。
- **空間系の一括取得**：Room/Space/Area の bulk 取得とパラメータ提案で解析が高速化。
